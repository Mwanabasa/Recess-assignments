...
from database import init_db, add_student, get_all_students, get_student_by_name, get_student_by_id, update_student, soft_delete_student
...
@app.route('/student/<int:student_id>/edit', methods=['GET'])
...
def update_student_route(student_id):
    ...
    success, message = update_student(student_id, student_data)
    if success:
        flash(message, 'success')
        return redirect(url_for('student_profile', name=student_data['student_name']))
    else:
        flash(message, 'error')
        return redirect(url_for('edit_student', student_id=student_id))
...
def delete_student(student_id):
    success, message = soft_delete_student(student_id)
    if success:
        flash(message, 'success')
    else:
        flash(message, 'error')
